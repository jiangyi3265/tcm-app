...
export const patientsApi = {
  ...
  merge(keepId, mergeId) { return request(`/api/patients/${keepId}/merge`, { method: 'POST', body: { mergeId } }) },
  signConsent(id) { return request(`/api/patients/${id}/consent`, { method: 'PATCH' }) },
  sendConsentEmail(id) { return request(`/api/patients/${id}/consent/send`, { method: 'POST' }) },
}
...
export const appointmentsApi = {
  ...
  status(id, status) { return request(`/api/appointments/${id}/status`, { method: 'PATCH', body: { status } }) },
  checkSlot(data) { return request('/api/appointments/check-slot', { method: 'POST', body: data }) },
  generateIntakeLink(id) { return request(`/api/appointments/${id}/intake-link`, { method: 'POST' }) },
}
...
export const consultationsApi = {
  ...
  paid(id, data = {}) { return request(`/api/consultations/${id}/paid`, { method: 'PATCH', body: data }) },
  dispense(id) { return request(`/api/consultations/${id}/dispense`, { method: 'PATCH' }) },
  ...
  generateReport(id) { return request(`/api/consultations/${id}/pdf/report`, { method: 'POST' }) },
  generateInvoice(id) { return request(`/api/consultations/${id}/pdf/invoice`, { method: 'POST' }) },
}
...
export const inventoryApi = {
  ...
  adjust(id, delta, reason) { return request(`/api/inventory/${id}/adjust`, { method: 'POST', body: { delta, reason } }) },
  adjustmentHistory(itemId) {
    const params = itemId ? `?itemId=${itemId}` : ''
    return request(`/api/inventory/adjustment-history${params}`)
  },
  batchImport(items) { return request('/api/inventory/batch-import', { method: 'POST', body: { items } }) },
  deductPrescription(herbals, prescriptionType) {
    return request('/api/inventory/deduct-prescription', { method: 'POST', body: { herbals, prescriptionType } })
  },
  restorePrescription(herbals, prescriptionType) {
    return request('/api/inventory/restore-prescription', { method: 'POST', body: { herbals, prescriptionType } })
  },
}
...
export const settingsApi = {
  ...
  updateServiceType(key, data) { return request(`/api/settings/service-types/${key}`, { method: 'PUT', body: data }) },
  addPriceList(data) { return request('/api/settings/price-lists', { method: 'POST', body: data }) },
  updatePriceList(id, data) { return request(`/api/settings/price-lists/${id}`, { method: 'PUT', body: data }) },
  deletePriceList(id) { return request(`/api/settings/price-lists/${id}`, { method: 'DELETE' }) },
}
...
export const filesApi = {
  upload(file, { patientId, consultationId, fileType } = {}) {
    ...
    return fetch('/api/files/upload', {
      ...
      if (!res.ok) throw new Error(payload.msg || payload.message || '上传失败')
      return payload
    })
  },
  listByPatient(patientId) { return request(`/api/files/patient/${patientId}`) },
  listByConsultation(consultationId) { return request(`/api/files/consultation/${consultationId}`) },
  remove(id) { return request(`/api/files/${id}`, { method: 'DELETE' }) },
  async open(path) {
    ...
    if (!res.ok) {
      const payload = await res.json().catch(() => ({}))
      throw new Error(payload.message || payload.error || '文件打开失败')
    }
    const blob = await res.blob()
    const url = URL.createObjectURL(blob)
...
export const consentPublicApi = {
  getInfo(token) { return request(`/api/consent/${token}`, { auth: false }) },
  sign(token, signatureName) { return request(`/api/consent/${token}/sign`, { method: 'POST', body: { signatureName }, auth: false }) },
}

export const intakePublicApi = {
  getInfo(token) { return request(`/api/intake/${token}`, { auth: false }) },
  submit(token, formData) { return request(`/api/intake/${token}/submit`, { method: 'POST', body: formData, auth: false }) },
}
