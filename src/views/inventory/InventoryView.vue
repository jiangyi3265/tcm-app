  ...
  <div class="inventory-view">
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane v-for="(config, key) in CATEGORY_CONFIG" :key="key" :label="config.label" :name="key">
          <el-table :data="getCategoryItems(key)" stripe>
            <el-table-column :label="t('inventory.itemName')" min-width="140">
            ...
            <el-table-column :label="t('inventory.natureTasteChannel')" width="160">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                ...
                <div v-else>
                  <div style="font-size: 12px">
                    ...
                    <span v-for="taste in (row.taste || [])" :key="taste" class="tcm-badge taste">{{ taste }}</span>
                  </div>
                  <div v-if="(row.guijing || []).length" style="font-size: 11px; color: #888; margin-top: 2px">
                    {{ t('inventory.channelShort') }}：{{ (row.guijing || []).join('、') }}
                  </div>
                </div>
              </template>
            </el-table-column>
            <el-table-column :label="t('inventory.currentStock')" width="130">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-input-number v-model="editForm.quantity" :min="0" :step="1" size="small" style="width: 110px" />
                </div>
            ...
            <el-table-column :label="t('inventory.alertLevel')" width="90">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-input-number v-model="editForm.minStockLevel" :min="0" size="small" style="width: 80px" />
                </div>
                <span v-else style="font-size: 13px; color: #888">{{ row.minStockLevel }} {{ row.unit }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="t('inventory.unitPrice')" width="100">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-input-number v-model="editForm.pricePerUnit" :min="0" :step="0.1" size="small" style="width: 90px" />
                </div>
                <span v-else>¥{{ row.pricePerUnit }}/{{ row.unit }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="t('inventory.supplier')" min-width="120">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-select v-model="editForm.supplierId" size="small" filterable clearable :placeholder="t('inventory.selectSupplier')" style="width:110px"
            ...
            <el-table-column v-if="key === 'powder'" :label="t('inventory.gramsPerBag')" width="75">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-input-number v-model="editForm.gramsPerPacket" :min="1" size="small" style="width: 70px" />
                </div>
                <span v-else>{{ row.gramsPerPacket ? row.gramsPerPacket + 'g' : '-' }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="t('inventory.toxicity')" width="65">
              <template #default="{ row }">
                <div v-if="editingId === row.id">
                  <el-select v-model="editForm.toxicity" size="small" style="width: 70px">
    ...
    <el-drawer v-model="showAddDialog" :title="t('inventory.addItem')" size="600px" direction="rtl" :close-on-press-escape="true">
      <el-form :model="newItem" label-width="90px" size="small">
        ...
        <el-row :gutter="12">
          ...
          <el-col :span="24">
            <el-form-item :label="t('inventory.functions')">
              <el-input v-model="newItem.functionsAndIndications" type="textarea" :rows="2" :placeholder="t('inventory.functionsPh')" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item :label="t('inventory.contraindications')">
              <el-input v-model="newItem.contraindications" :placeholder="t('inventory.contraindicationsPh')" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-drawer v-model="showDetailDialog" :title="detailItem?.name" size="500px" direction="rtl">
      <el-descriptions v-if="detailItem" :column="2" border size="small">
        ...
        <el-descriptions-item :label="t('inventory.taste')">{{ (detailItem.taste || []).join('、') || '-' }}</el-descriptions-item>
        <el-descriptions-item :label="t('inventory.channel')" :span="2">{{ (detailItem.guijing || []).join('、') || '-' }}</el-descriptions-item>
