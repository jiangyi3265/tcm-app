...
const startTimeOptions = computed(() => {
  ...
  const serviceDuration = SERVICE_TYPES[newAppt.value.serviceType]?.duration || 0
  const practitioner = practitioners.value.find((p) => p.id === newAppt.value.practitionerId) || null
  const weekdayKey = WEEKDAY_KEYS[dayjs(newAppt.value.date).day()]
  const workingHours = practitioner?.workingHours
  const hasConfiguredWorkingHours = workingHours && typeof workingHours === 'object' && Object.keys(workingHours).length > 0
  const range = hasConfiguredWorkingHours ? workingHours?.[weekdayKey] : defaultRange
  if (!range?.start || !range?.end || serviceDuration <= 0) return []

  const [startHour, startMinute] = String(range.start).split(':').map(Number)
  const [endHour, endMinute] = String(range.end).split(':').map(Number)
  if ([startHour, startMinute, endHour, endMinute].some(Number.isNaN)) return []

  const startTotal = startHour * 60 + startMinute
  const endTotal = endHour * 60 + endMinute
  const lastStart = endTotal - serviceDuration
  ...
  for (let total = startTotal; total <= lastStart; total += 15) {
    ...
    slots.push({ label: value, value, hour, minute })
  }
  return slots
})
...
const selectedStartTime = computed({
  get() {
    if (newAppt.value.startHour == null || newAppt.value.startMinute == null) return ''
    return `${String(newAppt.value.startHour).padStart(2, '0')}:${String(newAppt.value.startMinute).padStart(2, '0')}`
  },
  set(value) {
    if (!value) {
      newAppt.value.startHour = null
      newAppt.value.startMinute = null
      return
    }
    const [hour, minute] = value.split(':').map(Number)
    newAppt.value.startHour = Number.isNaN(hour) ? null : hour
...
const appointmentEndTimeLabel = computed(() => {
  ...
  return dayjs(newAppt.value.date)
    .hour(newAppt.value.startHour)
    .minute(newAppt.value.startMinute)
    .add(SERVICE_TYPES[newAppt.value.serviceType]?.duration || 0, 'minute')
    .format('HH:mm')
})
...
function syncSelectedStartTime() {
  ...
  if (!startTimeOptions.value.some((opt) => opt.value === selectedStartTime.value)) {
    selectedStartTime.value = startTimeOptions.value[0].value
  }
}
...
function resetNewAppt() {
  newAppt.value = {
    ...
    date: dayjs().format('YYYY-MM-DD'), startHour: null, startMinute: null, notes: '',
...
<style scoped>
.appointment-view { max-width: 100%; }
