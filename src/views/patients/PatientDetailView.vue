...
async function saveEdit() {
  try {
    await patientsStore.updatePatient(patientId, editForm.value)
    editing.value = false
...
const emailSim = useEmailSimulator()
...
async function sendConsentByEmail() {
  ...
  try {
    ...
    const emailData = emailSim.buildConsentEmail(patient.value, res.token, settingsStore.clinicName)
    emailSim.openEmailPreview(emailData)
    ElMessage.success(t('patientDetail.consentEmailSent'))
  } catch (e) {
...
const fileTree = computed(() => {
  ...
  for (const f of patientFiles.value) {
    const node = {
      ...
      url: f.filePath || f.url,
      size: f.fileSize,
      date: f.createTime || f.uploadedAt,
      isFile: true,
    }
        ...
        <div class="patient-basic">
          <div class="patient-name-row">
            ...
            <el-tag v-if="patient.consentPdfUrl" type="success" size="small" effect="plain"
              style="cursor:pointer" @click="ElMessage.info(t('patientDetail.consentPdfUrl') + patient.consentPdfUrl)">
              PDF
            </el-tag>
          ...
          <div v-if="patient.diseaseName" class="patient-disease">
            {{ t('patientDetail.diseaseLabel') }}{{ patient.diseaseName }}
          </div>
        </div>
      </div>
            ...
            <el-row :gutter="24">
              ...
              <el-col :span="14">
                ...
                <el-descriptions :column="1" border size="small" class="info-desc">
                  <el-descriptions-item :label="t('patientDetail.diseaseName')">{{ patient.diseaseName || '-' }}</el-descriptions-item>
                  <el-descriptions-item :label="t('patientDetail.historyAndMedication')">
                    <div style="white-space: pre-wrap; line-height: 1.6">{{ patient.historyAndMedication || '-' }}</div>
                  </el-descriptions-item>
              ...
              <el-col :span="10">
                ...
                <el-card shadow="never" class="consent-mini-card">
                  ...
                  <div v-if="!patient.consentSigned" style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap">
                    <el-button size="small" type="primary" plain @click="sendConsent">
            ...
            <el-row :gutter="24">
              <el-col :span="14">
                ...
                <el-form :model="editForm" label-width="110px" size="small">
                  ...
                  <el-form-item :label="t('patientDetail.diseaseName')">
